...
try{
  ...
  /*** UPDATE ***/
  if(isset($_POST['t'])){
    if(isset($_POST['mode']) && $_POST['mode'] == "JSON") {
    ...
    if(isset($_POST['input0500']) && $_POST['input0500']=='on'){
     ...
     /*** VERWERKEN CV ***/
    if (is_uploaded_file($_FILES['input020103']['tmp_name'])) {
      $sName = sys_get_temp_dir() . "/" . mktime() . "CV";
      move_uploaded_file($_FILES['input020103']['tmp_name'], $sName);
      $bData = file_get_contents($sName);
      $aAanmelding['personen']['vrijwilliger']['cv'] = base64_encode($bData);
      unlink($sName);
    }    
    ...
    if(isset($_POST['input0500']) && $_POST['input0500'] == 'on') {
      ...
      $oP = $oPage->createElement('p', 'Bedankt voor het invullen. We nemen contact met je op!');
